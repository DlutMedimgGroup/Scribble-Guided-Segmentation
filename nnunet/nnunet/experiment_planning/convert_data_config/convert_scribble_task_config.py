...
image的变换方式不变。分割标签转化为labels中的0000后缀数据。scribble转化为标签图后为labels中的0001后缀数据。
只有标签，无scribble，scribble全部像素为-1；只有scribble，无标签，标签全部像素为-1.
'''

def import_seed_data(file_name, size, origin, spacing):
    scribble_np = np.zeros(size[::-1])
    scribble_np[:] = -1
    ...
    ncrntLine = 0
    # 0 检查文件头
    if data[ncrntLine] != '#Anatomy Sketch Seed File\n':
        return -1
    ncrntLine += 1
    # 1 种子点数据数量
    if data[ncrntLine] != '#NUMBER OF DATA:\n':
        return -1
    ncrntLine += 1
    NumOfSeedData = int(data[ncrntLine])
    ncrntLine += 1
    # 2 DATALIST
    ...
    ncrntLine += 1
    ncrntID = 0
    for ncrntSeedData in range(NumOfSeedData):
        # 2.1 DATA LIST NUMBER
        if data[ncrntLine] != '#DATA LIST NUMBER ' + str(ncrntSeedData) + ' DEFINITION:\n':
            return -1
        ncrntLine += 1
        # 2.2 data_list_index
        ...
        if sublist[1] != 'data_list_index\n':
            return -1
        if int(sublist[0]) != ncrntSeedData:
            return -1
        # 2.3 data_name
        sublist = data[ncrntLine].split(' # ')
        ncrntLine += 1
        ...
        CtrColor = [int(subsublist[0]), int(subsublist[1]), int(subsublist[2])]
        # 2.5 Label
        sublist = data[ncrntLine].split(' # ')
        ncrntLine += 1
        if sublist[1] != 'data_label\n':
            return -1
        CtrLabel = int(sublist[0])
        # 2.6 number_of_Seed
        sublist = data[ncrntLine].split(' # ')
        ncrntLine += 1
        if sublist[1] != 'number_of_Line\n':
            return -1
        NumOfLine = int(sublist[0])
        SeedsLinelist = list()
        for ncrntSeedLine in range(NumOfLine):
            if data[ncrntLine] != '#Line LIST NUMBER ' + str(ncrntSeedLine) + ' DEFINITION:\n':
                return -1
            ncrntLine += 1
            # 2.6.1 line_index
            sublist = data[ncrntLine].split(' # ')
            ncrntLine += 1
            if sublist[1] != 'line_index\n':
                return -1
            if int(sublist[0]) != ncrntSeedLine:
                return -1
            # 2.6.2 line_ViewLabel
            sublist = data[ncrntLine].split(' # ')
            ncrntLine += 1
            if sublist[1] != 'line_ViewLabel\n':
                return -1
            crntViewLabel = int(sublist[0])
            # 2.6.3 line_dLayer
            sublist = data[ncrntLine].split(' # ')
            ncrntLine += 1
            if sublist[1] != 'line_dLayer\n':
                return -1
            crntLayer = float(sublist[0])
            # 2.6.4 line_nSerial
            sublist = data[ncrntLine].split(' # ')
            ncrntLine += 1
            if sublist[1] != 'line_nSerial\n':
                return -1
            crntnSerial = int(sublist[0])
            # 2.6.5 Points
            Pointlist = list()
            if data[ncrntLine] != '#START_POINTS_POS\n':
                return -1
            ncrntLine += 1
            while 1:
                sublist = data[ncrntLine].strip('\n').split(' ')
                ncrntLine += 1
                if len(sublist) != 3:
                    break
                sublist = np.array([float(x) for x in sublist])
                index = np.round((sublist-origin)/spacing).astype(np.int)
                scribble_np[index[2], index[1], index[0]] = CtrLabel
                # Output.append([ncrntID, float(sublist[0]), float(sublist[1]), float(sublist[2]),
                #                CtrLabel, crntViewLabel, crntnSerial])
            ncrntID += 1
    return scribble_np

def cat_and_split_label(file_base, size, origin, spacing, direction, label_path, scribble_path, output_folder):
    if (not isfile(label_path)) and (not isfile(scribble_path)):
        return False
    if isfile(label_path):
        ...
        shutil.copy(label_path, join(output_folder, 'labelsTr', file_base + "_0000.nii.gz"))
    else:
        label_np = np.zeros(size[::-1])
        label_np[:] = -1
        ...
        sitk.WriteImage(label_itk, join(output_folder, 'labelsTr', file_base + "_0000.nii.gz"))
    if isfile(scribble_path):
        # import .seed file and convert to label
        scribble_np = import_seed_data(scribble_path, size, origin, spacing)
        scribble_np = scribble_np[::int(direction[8]), ::int(direction[4]), ::int(direction[0])]
        scribble_itk = sitk.GetImageFromArray(scribble_np)
        scribble_itk.SetOrigin(origin)
        scribble_itk.SetSpacing(spacing)
        scribble_itk.SetDirection(direction)
        sitk.WriteImage(scribble_itk, join(output_folder, 'labelsTr', file_base + "_0001.nii.gz"))
    else:
        scribble_np = np.zeros(size[::-1])
        scribble_np[:] = -1
        scribble_itk = sitk.GetImageFromArray(scribble_np)
...
def cat_and_split(image_path, label_path, scribble_path, output_folder):
    ...
    if not cat_and_split_label(file_base, size, origin, spacing, direction, label_path, scribble_path, output_folder):
        # 如果不存在label和scribble，直接返回
        return
...
def cat_and_split_data(source_folder, output_task_name, num_processes=default_num_threads):
    ...
    path_scribbleTr = join(source_folder, 'scribbleTr')

    # names and folders
    full_task_name = source_folder.split("/")[-1]
    assert full_task_name.startswith("Scribble"), "The input folder must point to a folder that starts with ScirbbleXX_"
    first_underscore = full_task_name.find("_")
    assert first_underscore == 10, "Input folder start with ScirbbleXX with XX being a 2-digit id: 00, 01, 02 etc"
    ...
    for image_name in os.listdir(path_imagesTr):
        ...
        labels.append(join(path_labelsTr, base_name + '.nii.gz'))
        scribbles.append(join(path_scribbleTr, base_name + '_scribble.seed'))
        output_dirs.append(output_folder)

    # multi thread
    p = Pool(num_processes)
    p.starmap(cat_and_split, zip(images, labels, scribbles, output_dirs))
    p.close()
    p.join()
...
def main():
    ...
    assert folder.split('/')[-1].startswith("Scribble"), "This does not seem to be a scribble folder. Please give me a "\
                                                         "folder that starts with ScribbleXX and has the subfolders imagesTr, " \
    ...
    assert 'labelsTr' in subf, "This does not seem to be a scribble folder. Please give me a "\
                                                         "folder that starts with ScribbleXX and has the subfolders imagesTr, " \
                                                         "labelsTr, imagesTs and scribbleTr"
    assert 'scribbleTr' in subf, "This does not seem to be a scribble folder. Please give me a "\
                                                         "folder that starts with ScribbleXX and has the subfolders imagesTr, " \
                                                         "labelsTr, imagesTs and scribbleTr"
