...
def main():
    ...
    if network_trainer == 'nnUNetTrainerV2_obs':
        obs_size = config.getint('Task', 'obs_size')
    ...
    fold_mod = config.get('Task', 'fold_mod')
    if fold_mod == 's':
        fold_validation_path = config.get('Task', 'validation_path')
        if fold_validation_path == 'None':
            fold_validation_path = None
    else:
        fold_validation_path = None
    ...
    if network_trainer == 'nnUNetTrainerV2_obs':
        ...
        trainer = trainer_class(plans_file, fold, output_folder=output_folder_name,
                                ...
                                limited_tr_keys=limited_tr_keys, fold_mod=fold_mod, fold_validation_path=fold_validation_path)
